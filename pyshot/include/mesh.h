...
template <typename T> struct vec3d {
  ...
  template <typename Scalar> vec3d<T> &operator*=(const Scalar &s) {
    x *= s;
    y *= s;
    z *= s;
    return *this;
  ...
  template <typename Scalar> vec3d<T> operator/(const Scalar &v) const {
...
template <typename T> inline double magnitude(const vec3d<T> &v1) {
  return sqrt((v1.x * v1.x) + (v1.y * v1.y) + (v1.z * v1.z));
}

template <typename T> inline double normalize(vec3d<T> &p) {
...
public:
  ...
  oriented_point(const vec3d<double> &p, const vec3d<double> &nn)
      : vec3d<double>(p), n(nn) {}
};
...
public:
  ...
  void update_kd_tree() {
    ...
    flann_index = new flann::Index<flann::L2<double>>(
        *flann_data_mat, flann::KDTreeSingleIndexParams());
  ...
  void calc_normals() {
    ...
    for (size_t k = 0; k < np; ++k) {
      ...
      std::list<int>::const_iterator it = vert.tri_indices.begin(),
                                     it_end = vert.tri_indices.end();
      for (; it != it_end; ++it)
   ...
   */
  void nearest_neighbors_with_dist(int p, double radius,
    ...
    flann_index->radiusSearch(
        query, out_idx, out_dist, radius * radius,
    ...
    if (ns.front() != p) {
      // std::cout << "[WARNING] mesh_t::nearest_neighbors_with_dist(): The
      // first neighbor should be the query itself (point " << p << ")." <<
      // std::endl;
      neighs = ns;
      dists = ds;
    } else {
